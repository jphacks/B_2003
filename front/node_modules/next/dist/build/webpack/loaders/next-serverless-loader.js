...
pageIsDynamicRoute?`const nowParams = req.headers && req.headers["x-now-route-matches"]
        ...
        if (!renderMode) {
          if (_nextData || getStaticProps || getServerSideProps) {
            if (renderOpts.isNotFound) {
              ...
              const NotFoundComponent = ${absolute404Path?'notFoundMod.default':'Error'}
              ...
              const result = await renderToHTML(req, res, errPathname, parsedUrl.query, Object.assign({}, options, {
                getStaticProps: ${absolute404Path?`notFoundMod.getStaticProps`:'undefined'},
    ...
    export async function render (req, res) {
      try {
        ...
        if (html) {
          sendPayload(req, res, html, 'html', {generateEtags: ${JSON.stringify(generateEtags==='true')}, poweredByHeader: ${JSON.stringify(poweredByHeader==='true')}})
        }
      } catch(err) {
        console.error(err)
        await onError(err)
